...
void enqueue(Node *query, elem_t elem)
{
    list_push_back(query, elem);
}

elem_t dequeue(Node *query)
{
    return list_pop_front(query);
}

void delete(Node *q1, Node *q2)
{
    list_clear(q1);
    list_clear(q2);
    list_delete(q1);
...
int main()
{
    Node *q1 = calloc(1, sizeof(Node));
    Node *q2 = calloc(1, sizeof(Node));
    list_init(q1);
    list_init(q2);
    ...
    for (unsigned long i = 0; i < 1000000; i++)
    {
        if (list_is_empty(q1))
        {
            printf("second %lu", cnt);
            delete(q1, q2);
            return 0;
        }
        else if (list_is_empty(q2))
        {
            printf("first %lu", cnt);
            delete(q1, q2);
            return 0;
